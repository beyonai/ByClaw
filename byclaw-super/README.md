...
由于子 Agent 的过程和原始输出可能在前端被折叠，超级助手与专家团团长都必须在自己的最终答复中
交付可独立阅读的结果：汇总关键结论、已完成动作及失败或未决项；存在文件产物时，还必须在同一
答复中逐项列出子 Agent 返回的文件名和可用 URI/链接，并说明文件用途，不能只把结果留在子
Agent 消息中。
...
`metadata.project_info` 继续传递。子 Agent 会收到“当前任务关联以下项目环境”的明确提示；
项目信息（含 `workspace`）只作为背景，用于辅助理解用户请求和可用资源，
不得据此扩展、替换或改写用户原始意图，发生冲突时以用户明确要求为准。子 Agent
...
过去记住的配置。Run 只保存已选模型资源 ID；委派回调后的 Leader 恢复会按该 ID 读取
当前有效配置，因此密钥或模型参数更新不会因旧配置指纹导致整轮任务失败。若该模型资源
不可用，Run 会明确失败。发布前应在真实 PostgreSQL、Redis 和下游 Worker 上
验证滚动重启、进程强杀、网络中断及多轮回调，尤其是 Redis 持久化恢复和下游幂等行为。
