...
- **Pi model** (defaults to zhipu/glm-5.2): `PI_PROVIDER` + `PI_MODEL` must be set together or both omitted (mismatch is a startup error); `OPENAI_API_KEY` + `OPENAI_BASE_URL` for the Zhipu/GLM endpoint. `PI_ENTRY_MAX_BYTES`, `PI_SESSION_MAX_BYTES`, `PI_SESSION_MAX_ENTRIES`, `PI_SESSION_CACHE_DIR` / `PI_SESSION_CACHE_MAX_ENTRIES` / `PI_SESSION_CACHE_IDLE_TTL_MS`.
...
- **Execution tuning**: `RUN_LEASE_MS`, `RUN_QUEUE_POLL_MS`, `RUN_CREDENTIAL_MAX_TTL_MS`, `RUN_CREDENTIAL_CLEANUP_INTERVAL_MS`, `DELEGATION_TIMEOUT_MS`, `BYCLAW_INSTANCE_ID` (defaults to `byclaw-super-{hostname}-{pid}`, or `BYCLAW_WORKER_ID` if set; must be unique per instance).
