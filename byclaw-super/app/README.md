...
应用启动时装配 PostgreSQL 持久化 adapter、Pi Leader、KMS envelope credential cipher、
...
`POST /v1/sessions` 接收必填 `message`，返回 `sessionId + runId`。后续向
`POST /v1/sessions/:sessionId/runs` 提交新消息，会复用同一个 Pi LeaderSession 并返回新的
`runId`。创建、追加、查询、取消与订阅都必须携带 `Beyond-Token`。
...
- `Beyond-Token` 不写入 Run、Delegation、事件、Pi entries 或日志；它只以 KMS envelope
  encryption 密文短期保存，且只有当前 lease/fencing owner 可以读取；
...
持久 Worker binding、多实例 Run lease/fencing、KMS 密文凭证和 OpenClaw cursor resume。
上线前仍需提供真实 KMS adapter，并完成真实 PostgreSQL + Pi + OpenClaw 的分阶段 kill/failover
验收。Artifact 内容持久化属于后续里程碑。
