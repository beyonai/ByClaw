...
const BYAI_CHANNEL_ID = "byai-channel";
const BYAI_SDK_DIAGNOSTIC_SOURCE = "byai-channel-sdk";
const BYAI_INBOUND_SPAN_TTL_MS = 10 * 60 * 1000;
...
export type DiagnosticsOtelForcedContentCapture = {
  inputMessages?: boolean;
  outputMessages?: boolean;
  toolInputs?: boolean;
  toolOutputs?: boolean;
...
export type DiagnosticsOtelServiceOptions = {
  id?: string;
  exporterName?: string;
  includeDiagnosticSessionAttributes?: boolean;
  includeLangfuseSessionAttributes?: boolean;
  includeLangfuseUserAttributes?: boolean;
...
type LangfuseUserContext = {
  ...
  sessionUserIdsBySessionKey: Map<string, string>;
};
...
function isDiagnosticSessionAttributeKey(key: string): boolean {
  return (
    key === "openclaw.sessionId" ||
    key === "openclaw.session_id" ||
    key === "openclaw.sessionKey" ||
    key === "openclaw.session_key"
  );
}

function shouldDropOtelAttribute(
  key: string,
  options: Pick<DiagnosticsOtelServiceOptions, "includeDiagnosticSessionAttributes">,
): boolean {
  if (!DROPPED_OTEL_ATTRIBUTE_KEYS.has(key)) {
    return false;
  }
  return !(options.includeDiagnosticSessionAttributes && isDiagnosticSessionAttributeKey(key));
}

function redactOtelAttributes(
  attributes: Record<string, string | number | boolean>,
  options: Pick<DiagnosticsOtelServiceOptions, "includeDiagnosticSessionAttributes"> = {},
) {
  const redactedAttributes: Record<string, string | number | boolean> = {};
  for (const [key, value] of Object.entries(attributes)) {
    if (shouldDropOtelAttribute(key, options)) {
      continue;
    }
...
function readOtelSessionValue(value: unknown): string | undefined {
...
function normalizeLangfuseTraceAttributeId(value: string | undefined): string | undefined {
  if (!value) {
    return undefined;
  }
  if (/^[\x20-\x7E]{1,199}$/u.test(value)) {
    return value;
  }
  return `openclaw-${createHash("sha256").update(value).digest("hex").slice(0, 32)}`;
}

function normalizeLangfuseSessionId(value: string | undefined): string | undefined {
...
function resolveDefaultLangfuseUserId(options: DiagnosticsOtelServiceOptions): string | undefined {
  const envVar = options.langfuseUserIdEnvVar ?? "USER_CODE";
  return normalizeLangfuseTraceAttributeId(readOtelSessionValue(process.env[envVar]));
}

function resolveLangfuseUserId(
  evt: { userId?: unknown; sessionKey?: unknown },
  userContext: LangfuseUserContext,
): string | undefined {
  let userId = readOtelUserId(evt);
  const sessionKey = readOtelSessionValue(evt.sessionKey);
  if (userId && sessionKey) {
    userContext.sessionUserIdsBySessionKey.set(sessionKey, userId);
  } else if (!userId && sessionKey) {
    userId = userContext.sessionUserIdsBySessionKey.get(sessionKey);
  }
  if (!userId) {
    userId = userContext.defaultUserId;
  }
  return normalizeLangfuseTraceAttributeId(userId);
}

function assignDiagnosticSessionAttributes(
  attributes: Record<string, string | number | boolean>,
  evt: { sessionKey?: unknown; sessionId?: unknown },
  options: Pick<
    DiagnosticsOtelServiceOptions,
    "includeDiagnosticSessionAttributes" | "includeLangfuseSessionAttributes"
  >,
): void {
  if (!options.includeDiagnosticSessionAttributes && !options.includeLangfuseSessionAttributes) {
    return;
  }
  const sessionKey = readOtelSessionValue(evt.sessionKey);
  const sessionId = readOtelSessionValue(evt.sessionId);
  if (options.includeDiagnosticSessionAttributes) {
    if (sessionKey) {
      attributes["openclaw.sessionKey"] = sessionKey;
    }
    if (sessionId) {
      attributes["openclaw.sessionId"] = sessionId;
    }
  }
...
function assignLangfuseUserAttributes(
  attributes: Record<string, string | number | boolean>,
  evt: { userId?: unknown; sessionKey?: unknown },
  options: Pick<DiagnosticsOtelServiceOptions, "includeLangfuseUserAttributes">,
  userContext: LangfuseUserContext,
): void {
  if (options.includeLangfuseUserAttributes === false) {
    return;
  }
  const langfuseUserId = resolveLangfuseUserId(evt, userContext);
  if (langfuseUserId) {
    attributes["langfuse.user.id"] = langfuseUserId;
    attributes["user.id"] = langfuseUserId;
  }
}

function assignLangfuseTraceAttributes(
  attributes: Record<string, string | number | boolean>,
  evt: { sessionKey?: unknown; sessionId?: unknown; userId?: unknown },
  options: Pick<
    DiagnosticsOtelServiceOptions,
    | "includeDiagnosticSessionAttributes"
    | "includeLangfuseSessionAttributes"
    | "includeLangfuseUserAttributes"
  >,
  userContext: LangfuseUserContext,
): void {
  assignDiagnosticSessionAttributes(attributes, evt, options);
  assignLangfuseUserAttributes(attributes, evt, options, userContext);
  if (options.includeDiagnosticSessionAttributes) {
    const userId = resolveLangfuseUserId(evt, userContext);
    if (userId) {
      attributes["openclaw.userId"] = userId;
    }
  }
}
...
function readAssistantUsageFromModelCallContent(
  content: OtelModelCallContent | undefined,
): ModelCallUsageSnapshot | undefined {
  if (!content?.outputMessages) {
    return undefined;
  }
  const candidates = Array.isArray(content.outputMessages)
  ...
  for (let index = candidates.length - 1; index >= 0; index -= 1) {
    ...
    const input = nonNegativeFiniteNumber(raw.input);
    const output = nonNegativeFiniteNumber(raw.output);
    const cacheRead = nonNegativeFiniteNumber(raw.cacheRead ?? raw.cache_read);
    const cacheWrite = nonNegativeFiniteNumber(raw.cacheWrite ?? raw.cache_write);
    const total = nonNegativeFiniteNumber(raw.total ?? raw.totalTokens);
    const rawCost = raw.cost;
    const costUsd =
      typeof rawCost === "number"
        ? nonNegativeFiniteNumber(rawCost)
        : rawCost && typeof rawCost === "object" && !Array.isArray(rawCost)
          ? nonNegativeFiniteNumber(
              (rawCost as { totalUsd?: unknown; total?: unknown }).totalUsd ??
                (rawCost as { total?: unknown }).total,
            )
          : undefined;
    if (
      input === undefined &&
...
/** Langfuse GENERATION cost needs gen_ai.usage.* on the same span as gen_ai.request.model. */
function assignLangfuseGenAiUsageAttrs(
  attrs: Record<string, string | number | boolean>,
...
function applyForcedContentCapturePolicy(
  policy: OtelContentCapturePolicy,
  forced: DiagnosticsOtelForcedContentCapture | undefined,
): OtelContentCapturePolicy {
  if (!forced) {
    return policy;
  }
  return {
    ...policy,
    inputMessages: forced.inputMessages === true || policy.inputMessages,
    outputMessages: forced.outputMessages === true || policy.outputMessages,
    toolInputs: forced.toolInputs === true || policy.toolInputs,
    toolOutputs: forced.toolOutputs === true || policy.toolOutputs,
    systemPrompt: forced.systemPrompt === true || policy.systemPrompt,
    toolDefinitions: forced.toolDefinitions === true || policy.toolDefinitions,
  };
}
...
function readOtelToolCallContent(
  event: Record<string, unknown>,
  privateData?: Record<string, unknown>,
): OtelToolCallContent {
  const privateToolContent = privateData?.toolContent;
  const fromPrivate =
    privateToolContent && typeof privateToolContent === "object" && !Array.isArray(privateToolContent)
      ? (privateToolContent as OtelToolCallContent)
      : undefined;
  return {
    ...(fromPrivate?.toolInput !== undefined ? { toolInput: fromPrivate.toolInput } : {}),
    ...(fromPrivate?.toolOutput !== undefined ? { toolOutput: fromPrivate.toolOutput } : {}),
    ...(event.toolInput !== undefined ? { toolInput: event.toolInput } : {}),
    ...(event.toolOutput !== undefined ? { toolOutput: event.toolOutput } : {}),
  };
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    const assigned = assignOtelContentAttribute(
      attributes,
...
export function createDiagnosticsOtelService(
  ...
  const serviceId = options.id ?? "diagnostics-otel";
  const exporterName = options.exporterName ?? serviceId;
  ...
  return {
    ...
    async start(ctx) {
      ...
      if (protocol !== "http/protobuf") {
        ...
        ctx.logger.warn(`${serviceId}: unsupported protocol ${protocol}`);
        return;
      }
      ...
      if (!sdkPreloaded && (tracesEnabled || metricsEnabled)) {
        ...
        try {
          ...
          await stopStarted();
          ctx.logger.error(`${serviceId}: failed to start SDK: ${formatError(err)}`);
          throw err;
        }
      } else if (sdkPreloaded && (tracesEnabled || metricsEnabled)) {
        ctx.logger.info(`${serviceId}: using preloaded OpenTelemetry SDK`);
      }
        ...
        recordLogRecord = (evt, metadata) => {
          try {
            ...
            if (
              ...
              ctx.logger.error(`${serviceId}: log record export failed: ${formatError(err)}`);
            }
          }
        };
      }
      ...
      const spanWithDuration = (
        ...
        const endTimeMs = spanOptions.endTimeMs ?? Date.now();
        const startTime =
          typeof spanOptions.startTimeMs === "number"
            ? spanOptions.startTimeMs
            : typeof durationMs === "number" && durationMs >= 0
              ? endTimeMs - durationMs
              : undefined;
        const parentContext =
          "parentContext" in spanOptions ? (spanOptions.parentContext ?? undefined) : undefined;
      ...
      const setSpanAttrs = (
        ...
        span.setAttributes?.(redactOtelAttributes(attributes, options));
      };
      ...
      const byaiInboundParentContext = (entry: ActiveByaiInboundSpan | undefined) =>
        entry ? trace.setSpanContext(otelContextApi.active(), entry.span.spanContext()) : undefined;

      const removeActiveByaiInboundSpan = (entry: ActiveByaiInboundSpan) => {
        if (activeByaiInboundSpansBySessionKey.get(entry.sessionKey) === entry) {
          activeByaiInboundSpansBySessionKey.delete(entry.sessionKey);
        }
        for (const runId of entry.runIds) {
          if (activeByaiInboundSpansByRunId.get(runId) === entry) {
            activeByaiInboundSpansByRunId.delete(runId);
          }
        }
      };

      const endActiveByaiInboundSpan = (
        entry: ActiveByaiInboundSpan,
        endTimeMs: number,
        attrs: Record<string, string | number | boolean> = {},
        errorMessage?: string,
      ) => {
        setSpanAttrs(entry.span, attrs);
        if (errorMessage) {
        ...
        removeActiveByaiInboundSpan(entry);
        entry.span.end(endTimeMs);
      };

      const pruneExpiredByaiInboundSpans = (nowMs: number) => {
        for (const entry of activeByaiInboundSpansBySessionKey.values()) {
          if (nowMs - entry.lastSeenAtMs > BYAI_INBOUND_SPAN_TTL_MS) {
            endActiveByaiInboundSpan(entry, nowMs, {
              "openclaw.outcome": "skipped",
              "openclaw.reason": "byai_inbound_ttl_expired",
            });
          }
        }
      };

      const activeByaiInboundSpanForRun = (
        evt: { runId?: string; sessionKey?: string; ts: number },
      ) => {
        pruneExpiredByaiInboundSpans(evt.ts);
        const byRunId = evt.runId ? activeByaiInboundSpansByRunId.get(evt.runId) : undefined;
        if (byRunId) {
          byRunId.lastSeenAtMs = evt.ts;
          return byRunId;
        }
        const sessionKey = readOtelSessionValue(evt.sessionKey);
        if (!sessionKey) {
        ...
        const entry = activeByaiInboundSpansBySessionKey.get(sessionKey);
        if (entry && evt.runId) {
          entry.runIds.add(evt.runId);
          activeByaiInboundSpansByRunId.set(evt.runId, entry);
        }
        if (entry) {
          entry.lastSeenAtMs = evt.ts;
        }
        return entry;
      };
      ...
      const recordMessageReceived = (
        ...
        pruneExpiredByaiInboundSpans(evt.ts);
        const existing = activeByaiInboundSpansBySessionKey.get(sessionKey);
        if (existing) {
          endActiveByaiInboundSpan(existing, evt.ts, {
            "openclaw.outcome": "skipped",
            "openclaw.reason": "byai_inbound_replaced",
          });
        }
        const spanAttrs: Record<string, string | number | boolean> = {
          "openclaw.channel": lowCardinalityAttr(evt.channel),
        ...
        const byaiTraceId = readDiagnosticExtraString(evt, "byai.traceId");
        if (byaiTraceId) {
          spanAttrs["byai.traceId"] = byaiTraceId;
        }
      ...
      const recordMessageDispatchCompleted = (
        ...
        const sessionKey = readOtelSessionValue(evt.sessionKey);
        const entry = sessionKey ? activeByaiInboundSpansBySessionKey.get(sessionKey) : undefined;
        if (!entry) {
          return;
        }
        entry.lastSeenAtMs = evt.ts;
        if (evt.outcome === "error" || entry.runIds.size === 0) {
          endActiveByaiInboundSpan(
            entry,
            evt.ts,
            {
              "openclaw.outcome": evt.outcome,
            ...
            evt.outcome === "error" ? (evt.error ?? evt.reason ?? "byai dispatch error") : undefined,
          );
        }
      };
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          ...
          spanWithDuration("openclaw.run", spanAttrs, undefined, {
            parentContext:
              activeTrustedParentContext(evt, metadata) ?? byaiInboundParentContext(byaiInboundSpan),
            startTimeMs: evt.ts,
          }),
        );
        const parentSpanId = correlatedTraceContext(evt, metadata)?.parentSpanId;
      ...
      const recordRunCompleted = (
        ...
        const finishByaiInboundRun = () => {
          if (!byaiInboundSpan) {
            return;
          }
          if (evt.runId) {
            byaiInboundSpan.runIds.delete(evt.runId);
            if (activeByaiInboundSpansByRunId.get(evt.runId) === byaiInboundSpan) {
              activeByaiInboundSpansByRunId.delete(evt.runId);
            }
          }
          byaiInboundSpan.lastSeenAtMs = evt.ts;
          if (byaiInboundSpan.runIds.size === 0) {
            endActiveByaiInboundSpan(
              byaiInboundSpan,
              evt.ts,
              {
                "openclaw.outcome": evt.outcome,
                "openclaw.duration_ms": evt.ts - byaiInboundSpan.startedAtMs,
              },
              evt.outcome === "error" ? (evt.errorCategory ?? "byai run error") : undefined,
            );
          }
        };
        if (trackedSpan && trustedTrace?.spanId) {
        ...
        try {
          ...
          switch (evt.type) {
            ...
            case "tool.execution.started":
              recordToolExecutionStarted(evt, metadata, privateData);
              return;
            case "tool.execution.completed":
              recordToolExecutionCompleted(evt, metadata, privateData);
              return;
            case "tool.execution.error":
              recordToolExecutionError(evt, metadata, privateData);
              return;
          ...
          ctx.logger.error(`${serviceId}: event handler failed (${evt.type}): ${formatError(err)}`);
        }
      });
      ...
      unregisterUnhandledRejectionHandler = registerUnhandledRejectionHandler((reason) => {
        ...
        ctx.logger.warn(
          `${serviceId}: suppressed OTLP exporter unhandled rejection (code=${String(code)})`,
        );
        return true;
      });
      ...
      if (logsEnabled) {
        ctx.logger.info(`${serviceId}: logs exporter enabled (OTLP/Protobuf)`);
      }
    },
    async stop() {
      await stopStarted();
