...
`BYAI_WORKER_ID` 可以保持固定，同一时刻只能有一个活跃实例占用同一 ID；多个副本必须各有独立 ID。无显式设置时仍沿用按用户生成的原有 ID。

通道停止时会等待 SDK runner 取消在途执行、停止心跳、释放独占租约并关闭连接后，才向 OpenClaw 报告停止完成。启动中收到停止信号也会收尾，不再遗漏 abort 或后台发起清理后立即返回。启动失败会传给宿主，避免通道看似运行但实际不消费任务。

异常退出无法执行清理时，新实例只针对 `worker_id already in use` 等待重试，每秒一次、单次启动最多等待 65 秒；旧租约自然失效或旧实例正常释放后自动继续。该窗口覆盖 SDK 初次认领的 60 秒租约，正常心跳续租使用现有共享 Redis 配置。其他初始化错误立即上报。等待期间停止可立即取消；仍在运行并续租的同名实例不会被强制接管。失败实例清理时也禁止 SDK 1.5.3 的无所有权租约删除回退。

部署应给优雅退出留足在途任务取消和连接释放时间；强制终止后允许租约等待窗口，不要因短时间未就绪反复杀进程。此机制恢复 worker 注册，不代表中断任务已完成业务恢复。

定向验证：`npm test -- src/worker-lifecycle.test.ts src/sdk-app.test.ts`，然后 `npm run build`。
