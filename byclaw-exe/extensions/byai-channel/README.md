...
develop 移植回归（基线 `7280f3fc3`）：早期模型配置与三次恢复提交 `fe40b7b13` 已在目标分支，只补入 D0.4.2 的加固和国际化提交 `3f8cf5135`，保留 develop 现有冷启动消费控制。两个插件构建通过；8 个定向套件 101 项、模型配置相关 50 项通过。channel 完整套件执行 369 项，364 项通过，5 项既有失败；另有 `prompt-injection-snapshot.test.ts` 的 `beforeEach` 未定义及 `session-key.test.ts` 引用缺失模块两个加载错误。这些失败和加载错误在移植前后均可复现，没有新增失败；本次未扩展修改到无关测试。
