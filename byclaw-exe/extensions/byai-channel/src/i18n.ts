...
function resolveBaiyingCallVerb(
  language: string | undefined,
  args: Record<string, any> | undefined,
): string {
  ...
  if (["OBJECT", "VIEW"].includes(type)) {
    return en ? "Query" : "查询";
  }
  if (["AGENT", "A2A", "INTERFACE", "PAGE"].includes(type)) {
    return en ? "Request" : "请求";
  }
...
function buildBaiyingCallToolStartTitle(
  language: string | undefined,
  args: Record<string, any> | undefined,
): string {
  const en = isEnglishLanguage(language);
  ...
  if (!resource && !purpose) {
    const verb = resolveBaiyingCallVerb(language, args);
    return en ? `${verb} resource ${BAIYING_CALL_TOOL_TAG}` : fallback;
  }

  const verb = resolveBaiyingCallVerb(language, args);
  if (resource) {
    return en
      ? `${verb} "${resource}" ${BAIYING_CALL_TOOL_TAG}`
      : `${verb}「${resource}」${BAIYING_CALL_TOOL_TAG}`;
  }
  return en
    ? `${verb}: ${purpose} ${BAIYING_CALL_TOOL_TAG}`
    : `${verb}：${purpose}${BAIYING_CALL_TOOL_TAG}`;
}

function buildBaiyingCallToolResultTitle(
  language: string | undefined,
  params: {
