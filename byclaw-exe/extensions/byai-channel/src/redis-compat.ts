...
export function resolveRedisCompatConfig(
  ...
  const password = env.REDIS_PASSWORD !== undefined && env.REDIS_PASSWORD !== ""
    ? env.REDIS_PASSWORD
    : undefined;
  ...
  if (!standaloneHost || !Number.isFinite(port) || !Number.isFinite(db)) {
    return null;
  }
  return { mode, host, port, db, username, password, clusterNodes };
}
...
export function assertRedisKeySchemaForMode(
  config = resolveRedisCompatConfig(),
  env: NodeJS.ProcessEnv = process.env,
): void {
  if (config?.mode === "cluster" && schemaVersion(env) !== "v2") {
...
function versioned(v1Key: string, v2Suffix: string): string {
  return schemaVersion() === "v2" ? `${V2_PREFIX}${v2Suffix}` : v1Key;
}
...
export const RedisCompatKeys = {
  ...
  sessionDataStreamScanPattern: () => schemaVersion() === "v2"
...
export function createRedisCompatClient(
  options: RedisCompatClientOptions = {},
): RedisCompatClient {
  const config = resolveRedisCompatConfig();
...
export function createByFrameworkRedisClient(
  options: RedisCompatClientOptions = {},
): RedisCompatClient {
...
export async function closeRedisCompatClient(
  client: RedisCompatClient | null | undefined,
): Promise<void> {
  if (!client) return;
  try {
    await client.quit();
...
export async function scanRedisCompatKeys(
  ...
  const nodes = typeof (client as { nodes?: unknown }).nodes === "function"
    ? (client as Cluster).nodes("master")
    : [client as Redis];
