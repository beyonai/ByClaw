...
function renderMetadataMarkdown(params: {
  ...
  return [
    ...
    "Read skills directly from these authoritative paths. Do not copy, install, symlink, or materialize them into public Claude Code or Codex skill directories.",
...
function renderClientInstructions(params: {
  ...
  const common = [
    ...
    "3. Read every Linked Skill directly from its metadata `skillDocPath` from byte 0 through EOF before accessing the query.",
    "4. Do not copy, install, symlink, or materialize Linked Skills into public Claude Code or Codex skill directories.",
    "5. Load every other resource that metadata makes mandatory for the current task, including future resource types unknown to this client.",
    "6. Write `bootstrap-receipt.json`; if any mandatory rule, Linked Skill, or resource cannot be satisfied, write BLOCKED and do not read `query.md`.",
    "7. Only after the receipt status is READY, read `query.md` and treat it as the user request.",
    "8. After READY, apply every loaded Linked Skill whose trigger conditions match the query; acknowledging or summarizing a skill does not count as using it.",
    "9. Only after READY, use `plan-bundle.json` when machine-readable detail is needed; it contains the protected user input.",
    "10. Do not invent agents, skills, or business rules that are absent from metadata.",
    "11. Preserve proof: cite loaded rule/resource paths and command output, and list which roster member handled each work item.",
    "12. Follow the Fixed Work Specs below; they are mandatory for all downstream agents and subagents.",
    "",
    "## Fixed Work Specs",
    "",
    params.fixedWorkSpecs.sessionFiles.policyMarkdown,
  ...
  if (params.clientType === ACP_CLIENT_TYPES.codex) {
    return [
      ...
      "- Load Linked Skills from the absolute `skillDocPath` values in metadata; do not depend on `.agents/skills` or legacy `.codex/skills` discovery.",
      "- A Linked Skill missing from Codex's default skill list is not a reason to skip it.",
  ...
  return [
    ...
    "- Load Linked Skills from the absolute `skillDocPath` values in metadata; do not depend on `.claude/skills` discovery.",
    "- A Linked Skill missing from Claude Code's default skill list is not a reason to skip it.",
...
function buildTask(params: {
  ...
  return [
    ...
    "5. 必须直接读取权威路径；禁止将 Linked Skills 复制、安装、软链接或物化到 Claude Code、Codex、项目级或用户级公共 skills 目录。",
