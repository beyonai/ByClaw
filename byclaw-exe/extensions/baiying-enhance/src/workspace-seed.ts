...
function getFirstAgentItem(raw: unknown): BaiyingAgentItem | null {
  ...
  const relResourceList = pickRelResourceList(o);
  return { ...(first as BaiyingAgentItem), relResourceList } as BaiyingAgentItem;
}
...
function getRawDetailItem(raw: unknown): BaiyingAgentItem | null {
  ...
  return {
    ...
    relResourceList: pickRelResourceList(d),
    coreCompetencies,
  };
}
...
export function buildToolsMd(item: BaiyingAgentItem, fallbackAgentId?: string): string {
  ...
  lines.push(
    ...
    "- `SCENE`, `OBJECT`, and `VIEW` resources are dispatched through SDK `callAgent` to `BYCLAW_DATA`; `baiying_call` fills `call_scene_ids` / `call_object_ids` / `call_view_ids` from selected resource codes and forwards `ontology_base_code` for a single base, or `ontology_resources` / `ontology_base_resource_groups` for multiple bases or multiple ontology resources.",
    "- For large `SCENE`/`OBJECT`/`VIEW` results, backend may return `file_url`, and `file_url` is a local file path; treat it as the authoritative full payload and use this local path for downstream business processing.",
