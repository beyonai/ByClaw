...
两个实体命令都支持可选参数 `--session-id SESSION_ID`。当前任务上下文存在会话 ID 时，必须显式传入 `--session-id`，不得省略或依赖运行环境代为发现，以便向对应会话空间发送实体文件变更通知。只有当前任务上下文没有会话 ID 时才可省略；此时 CLI 尝试使用运行环境中的当前会话，仍无法确定会话时，实体处理任务照常提交，但不会向会话空间发送实体文件变更通知。
