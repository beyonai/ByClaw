...
```bash
node /app/scripts/by-knowledge-manager.mjs help
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs <command> [options]
```

For commands that need local files, pass a file path that is readable from the runtime environment:

```bash
node /app/scripts/by-knowledge-manager.mjs upload \
...
```bash
node /app/scripts/by-knowledge-manager.mjs list --resource-id RESOURCE_ID --directory-path /
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs mkdir --resource-id RESOURCE_ID --directory-path / --directory-name 产品资料
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs rename-dir --resource-id RESOURCE_ID --directory-path /产品资料 --directory-name 产品手册
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs check-conflicts --resource-id RESOURCE_ID --directory-path /产品资料 --file-name a.md
```

Upload can accept repeated `--file-path` values. Successful upload automatically triggers build for the returned files.

```bash
node /app/scripts/by-knowledge-manager.mjs upload --resource-id RESOURCE_ID --directory-path /产品资料 --file-path /tmp/a.md --check-conflicts
```

Then list the upload directory and confirm the uploaded file appears:

```bash
node /app/scripts/by-knowledge-manager.mjs list --resource-id RESOURCE_ID --directory-path /产品资料
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs update-file --resource-id RESOURCE_ID --directory-path /产品资料 --file-path /tmp/a.md
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs build --resource-id RESOURCE_ID --file-path /产品资料/a.md
```

Check status:

```bash
node /app/scripts/by-knowledge-manager.mjs build-status --resource-id RESOURCE_ID --file-path /产品资料/a.md
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs download --resource-id RESOURCE_ID --file-path /产品资料/a.md --output /tmp/a.md
```

Download a directory archive:

```bash
node /app/scripts/by-knowledge-manager.mjs download --resource-id RESOURCE_ID --directory-path /产品资料 --output /tmp/产品资料.zip
```

Read file content by line range:

```bash
node /app/scripts/by-knowledge-manager.mjs read-file --resource-id RESOURCE_ID --file-path /产品资料/a.md --start-line 1 --end-line 80
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs search --resource-id RESOURCE_ID --query "员工请假流程是什么" --top-k 5
```

Search multiple knowledge bases by repeating `--resource-id`:

```bash
node /app/scripts/by-knowledge-manager.mjs search --resource-id RESOURCE_ID_A --resource-id RESOURCE_ID_B --query "员工请假流程是什么" --top-k 10
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs remove-file --resource-id RESOURCE_ID --file-path /产品资料/a.md
```
...
```bash
node /app/scripts/by-knowledge-manager.mjs delete-dir --resource-id RESOURCE_ID --directory-path /产品资料
```
