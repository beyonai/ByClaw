...
采集编排器 `knowledge-collection` 拥有入库编排与确认边界；`by-knowledge-manager` 是唯一的底层入库执行器。
`scripts/ingest.mjs` 只生成可校验的交接单，不启动管理器 CLI；Agent 必须读取 `by-knowledge-manager` skill 并按交接单执行，
不得把入库所有权移回 `bycli` 或改由采集脚本直接写知识库。
...
- `ingest` 与 `upload-doc` 执行前，都必须展示解析后的目标目录并获得用户明确确认；`ingest` 交接单采用的
...
入库结果必须逐篇映射回 inventory `itemId`。只有 `by-knowledge-manager` 返回上传成功且对应 build 请求被接受时才记为 success；
不等待异步索引完成。批量失败且无法确认逐篇结果时记为 unknown，不得清理对应工作副本。Agent 根据交接单的 `resultContract` 映射
管理器 JSON 后，通过后处理状态脚本写入 run。只有输入文件名在批次内唯一、上传结果能唯一对应该文件名，并且 build 精确引用该上传
结果路径时，才可机械映射为 success；任一条件不满足必须返回 `unknown`，不得用模糊文件名匹配猜测 success。`upload-doc` 保持
独立直传能力并直接返回 `itemResults`。
...
- `ingest`：将用户选中的 Markdown 文件、Markdown 目录或规范化采集结果规范化为 `by-knowledge-manager` 交接单，
  不执行 upload/build；Agent 随后加载 `by-knowledge-manager` 并执行。知识库目标使用 `--knowledge-base-resource-id`，也兼容
...
- 交接单的 `manager.checkConflicts=true` 时，Agent 先让管理器检查冲突。用户明确确认全部 `overwritePaths` 后，
  Agent 再以 `update-file` 对每个文件逐一执行；必须重新检查得到的冲突集合与确认值完全一致。恢复执行不得重新上传
  已经成功处理的图片、音频或视频。
