...
当用户要求采集一篇指定标题的文章时，必须先区分“逐字匹配”和“高度相关候选”：

- 搜索结果包含用户标题的逐字匹配：可以继续按来源路由下载并采集；
- 只有高度相关候选、没有逐字匹配：必须先把候选标题、来源、URL 和匹配理由返回给用户，必须先询问用户确认“是否采集这篇候选文章？”；
- 未确认不得下载：在用户明确确认前，不得调用 `download`、`web read`、`materialize` 或其他正文获取执行器，也不得把候选当成目标文章登记入库；
- 用户未确认、拒绝或候选存在歧义时，停止在发现阶段，并如实报告未完成正文采集。
