...
Every artifact for one collection task must remain beneath that task's initialized session directory. If a delegated tool needs a staging path, use the session's `raw/` subtree; then register or materialize the result into `markdown/items/` and `sanitized/items/`. When an approved source record contains article media, preserve the source response as raw evidence and put local preview copies beside `sanitized/items/<article-name>-<item-id>/index.md` in `assets/`; do not leave the delivery dependent on remote image links or duplicate already-recorded media URLs into `raw/`. Do not create a sibling delivery directory such as `<topic>-fulltext/` or `<topic>-articles/`. Duplicate records, partial materialization, or a delegated-tool failure do not waive this requirement: retain the raw evidence and mark the affected inventory item `pending` or `failed` in the same session.
