...
2. For candidate-only public URL discovery, run `public-discover`. 路由先看交付物和是否指定数量：用户明确只要候选链接时，即使用户指定了链接数量，也使用 `public-discover`；用户要求文章、正文、全文或落盘内容但未指定数量时，默认使用 `unified-search`，并行检索公共互联网与当前项目云盘，再用 `unified-materialize` 物化选中的两类正文；其中“文章”按完整正文处理。执行 `unified-search` 时，必须把可信 `<project_context>` 中的 `project_id` 透传为 `--project-id`，并把 `project-context basic` 返回的 `project.cloudResourceId` 传给首次 `init --cloud-resource-id`（CLI 会自动生成根目录 `cloudDiscoveryScope`），随后再调用 `unified-search --project-id`；不要手工拼接或猜测 scope JSON。用户明确要求数量（如“一篇”“5 篇”“至少 10 篇”）时，改用 `public-collect`，只检索公共互联网并执行数量闭环。用户明确限定来源时服从限定，不自动添加其他来源。Its `online-search` channel concurrently uses configured Tencent WSA and Search1API capabilities, falling back to SearXNG when both are unavailable or fail. TypeSafe Jev is optional: `TYPESAFE_API_KEY` enables bounded query planning and post-filter candidate reranking; a missing token, disable switch, timeout, invalid response, or provider failure preserves the existing query and ordering. Provider and Jev secrets must never enter arguments, snapshots, session state, logs, or diagnostics. Jev scores cannot authorize URLs or replace page, topic, or full-text verification. WSA 与 Search1API 返回的搜索摘要只是发现证据，不是文章正文，也不保证目标页不会是登录、注册、验证、错误或导航页面。`pageType`, `weak`, and `articleCandidateIds` remain compatibility classifications; persisted `discoveryDisposition=probe` controls whether a page may be read.

   Jev 效率策略由 Runner 自动执行，详见 [online-search.md](references/online-search.md)：批量结合相关性、全文可获取性和疑似重复重排；连续两次未交付后按本轮反馈重新调度；中文文章渠道在既有来源内优先选择；只有缺标题或主题 unknown 的模糊正文才选择已有证据并重新执行本地规则。不得要求根 Agent 逐篇额外调用 Jev。`TYPESAFE_ENABLED=false` 可关闭，未配置或失败自动回退。

   统一搜索、云盘元数据推荐、研究 follow-up 优先级、超预算上下文选择和选取式站点队列也由 Runner 提供可选 Jev 增强。无效、失败或低置信度结果必须整体降级到原有逻辑，禁止应用半批结果。内部资料仅在 `TYPESAFE_ENTERPRISE_ENABLED=true` 时可发给 Jev。`plan` / `branch` 返回 `suggestedFollowups` 时可据此安排已有问题，但不能省略用户要求的范围与研究深度；全部归档不得凭模型删减页面。

   新增效率扩展同样由 Runner 自动执行：通用合法来源计划最多 3 个来源一波，`public-collect` 先验证本波全文再决定下一波；第二轮查询仅从保留主题的既有模板中选择。IMA 知识库、钉钉目录和站点 seed/大队列只重排已授权条目，`all` 不裁剪。单候选跳过模型，同一次调用链采用短时结果复用和服务故障熔断；不跨任务缓存。`JEV_PROVIDER_SELECTION_ENABLED=true` 才开启在线提供商组合实验，默认仍并行 WSA + Search1API；实验选择无效恢复原并行路径。完整条件和降级见 [online-search.md](references/online-search.md)，不得据此声称已经获得实网提速。
