...
async function testRewriteImageLinksRemainsAvailableWithStateMachine() {
  const help = await runCli(['--help']);
  assert.equal(help.code, 0, help.stderr || help.stdout);
  assert.equal(
    help.json?.commands?.['rewrite-image-links'],
    '把 sanitized 正文里的本地图片相对链接改写为 fileBrowser 下载 URL',
  );
  ...
  try {
    ...
    const mapFile = join(root, '.post-processing-inputs/image-link-map.json');
    await writeFile(mapFile, JSON.stringify({
      linkMap: {
        'images/img_001.png': '/byaiService/datasetController/download?resourceId=42&directoryPath=%2Fimg_001.png',
      },
    }));
    ...
    assert.match(rewritten, /!\[封面\]\(\/byaiService\/datasetController\/download\?/);
...
async function testRewriteImageLinksRejectsUnsafeLinkMap() {
  ...
  try {
    ...
    const mapFile = join(root, '.post-processing-inputs/link-map.json');
    const cases = [
...
await testRewriteImageLinksRemainsAvailableWithStateMachine();
