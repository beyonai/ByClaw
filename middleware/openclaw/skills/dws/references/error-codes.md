...
- 认证失败（`AUTH_TOKEN_EXPIRED` / `USER_TOKEN_ILLEGAL` / 401/403）— OpenClaw 中必须先用 `exec` 执行 `dws auth login --device`，再立即用 `process` 读取本次执行返回
- 登录链接提取 — `login_url` 只能取本次 `process` 输出中“或者直接打开以下链接：”下一行的完整 URL（必须包含 `user_code` 查询参数），不得使用普通 `链接:`、历史输出、拼接或猜测出的链接
- 认证失败后返回客户端 — 成功提取链接时必须返回：`[点击登录钉钉](<login_url>)`；若未提取到“直接打开”的完整链接，需明确报错并提示用户手工执行 `dws auth login --device`
