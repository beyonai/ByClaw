...
class FakeApi:
    ...
    def build_object_instances(self, *, instance_ids: list[str], batch_size: int) -> dict[str, str]:
        ...
        self.build_requests = getattr(self, "build_requests", [])
        self.build_requests.append((instance_ids, batch_size))
        if getattr(self, "build_should_fail", False):
            raise ValueError("build service unavailable")
        return {"status": "accepted"}


class TwoAdsObjectApi(FakeApi):
    """One ODS plus two ADS objects, so an object whitelist is observable."""
    ...
    def list_authorized_resources(self, _employee_resource_id: str, _page: int) -> dict[str, object]:
        return {
            ...
            "list": [
                {"resourceBizType": "OBJECT", "resourceCode": "raw_doc"},
                {"resourceBizType": "OBJECT", "resourceCode": "concept"},
                {"resourceBizType": "OBJECT", "resourceCode": "product"},
            ],
        }

    def get_object(self, object_code: str) -> dict[str, object]:
        domain = "ods" if object_code == "raw_doc" else "ads"
        names = {"raw_doc": "原始文档", "concept": "概念", "product": "产品"}
        return {
            "objectCode": object_code,
            "objectName": names[object_code],
            "objectDesc": f"{domain} object",
            "properties": [{"propertyCode": "title"}],
            "extProperty": {"use_domain": domain},
        }
    ...
    def extract_fragments(self, **kwargs: object) -> dict[str, object]:
        self.extract_kwargs = kwargs
        if self.extract_should_fail:
            raise ValueError("extraction unavailable")
        return {
            "fragments": [
                {"object_code": "concept", "entity_name": "智能客服", "content": "智能客服支持多轮对话。"}
            ]
        }
...
class KnowledgeOrganizerInitializationTests(unittest.TestCase):
    ...
    def _ingest_one(self, name: str = "智能客服说明") -> None:
        source = Path(self.temp_dir.name) / f"{name}.md"
        source.write_text("# 智能客服\n支持多轮对话", encoding="utf-8")
        self.organizer.ingest(
            self.task_dir,
            source=source,
            object_code="raw_doc",
            storage_file_name=f"{name}.md",
            labels={"title": "智能客服"},
        )

    def test_build_retries_a_previously_failed_batch(self) -> None:
        self.organizer.initialize(self.task_dir, "employee-1")
        self._ingest_one()
        self.organizer.organize(self.task_dir)
        self.api.build_should_fail = True

        failed = self.organizer.build(self.task_dir)
        self.api.build_should_fail = False
        retried = self.organizer.build(self.task_dir)

        self.assertEqual(failed[0]["status"], "failed")
        self.assertEqual(retried[0]["status"], "accepted")
        self.assertEqual(retried[0]["instance_ids"], ["ads-term-1"])
        self.assertEqual(len(self.api.build_requests), 2)

    def test_build_skips_instances_already_accepted(self) -> None:
        self.organizer.initialize(self.task_dir, "employee-1")
        self._ingest_one()
        self.organizer.organize(self.task_dir)

        self.organizer.build(self.task_dir)
        second = self.organizer.build(self.task_dir)

        self.assertEqual(second, [])
        self.assertEqual(len(self.api.build_requests), 1)

    def test_ingest_is_idempotent_for_identical_content_and_object(self) -> None:
        self.organizer.initialize(self.task_dir, "employee-1")
        source = Path(self.temp_dir.name) / "notes.md"
        source.write_text("# 标题\n正文", encoding="utf-8")
        ...
        state = self.organizer._load_state(self.task_dir)
        self.assertEqual(len(state["ingestions"]), 1)

    def test_organize_resume_reuses_the_saved_object_scope(self) -> None:
        # Two ADS objects are required: with only one, a lost whitelist and a
        # honored whitelist would produce identical ads_objects.
        api = TwoAdsObjectApi()
        organizer = knowledge_organizer.KnowledgeOrganizer(api)
        organizer.initialize(self.task_dir, "employee-1")
        source = Path(self.temp_dir.name) / "notes.md"
        ...
        api.extract_should_fail = True

        organizer.organize(self.task_dir, {"concept"})
        self.assertEqual(set(api.extract_kwargs["ads_objects"]), {"concept"})
        state = organizer._load_state(self.task_dir)
        self.assertEqual(state["organize_object_codes"], ["concept"])

        api.extract_should_fail = False
        organizer.organize(self.task_dir, resume=True)

        self.assertEqual(set(api.extract_kwargs["ads_objects"]), {"concept"})

    def test_concurrent_files_share_one_instance_for_the_same_new_entity(self) -> None:
        class SharedEntityApi(ConcurrentFakeApi):
            def __init__(self) -> None:
                super().__init__()
            ...
            def extract_fragments(self, **kwargs: object) -> dict[str, object]:
                super().extract_fragments(**kwargs)
                return {
                    "fragments": [
                        {"object_code": "concept", "entity_name": "共享实体", "content": "关于共享实体的知识。"}
                    ]
                }

            def create_entity(self, *, object_code: str, entity_name: str) -> str:
                with self._lock:
                    self.created_entities.append((object_code, entity_name))
                    self.next_entity_index += 1
                    return f"ads-term-{self.next_entity_index}"

        api = SharedEntityApi()
        organizer = knowledge_organizer.KnowledgeOrganizer(api)
        organizer.initialize(self.task_dir, "employee-1")
        for index in range(4):
            source = Path(self.temp_dir.name) / f"shared-{index}.md"
            source.write_text(f"document-{index}", encoding="utf-8")
            organizer.ingest(
                self.task_dir,
                source=source,
        ...
        self.assertEqual(api.created_entities, [("concept", "共享实体")])
        self.assertEqual({fragment["instance_id"] for fragment in fragments}, {"ads-term-1"})
