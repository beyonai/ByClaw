...
开始任何任务前，必须先阅读并遵守 [企业微信助手运行政策](references/operating-policy.md)。该政策适用于本目录下所有子 skill，并优先于子 skill 中的示例和局部规则。
...
先按错误语义分类，再决定是否授权恢复。仅当 `wecom-cli` 命令返回 `Error` / `ERROR` / 非 0 退出，且错误信息明确包含或近似表达以下授权状态时，才进入本流程：
...
- `errcode: 850001`、`invalid api key`、API Key 无效/错误或机器人文档配置不正确
- 参数格式错误、对象不存在、权限不足、接口不支持、网络错误或其他业务错误

遇到上述配置/业务错误时，停止 `init` 尝试，按错误类型给出客观提示；其中 `invalid api key` 应建议管理员检查机器人 API Key 和文档使用配置，不得声称“未授权”，也不得伪造授权链接。
...
2. 运行：
   ...
   ```
   必须使用后台/长运行会话方式执行，授权等待超时设置为至少 **120 秒**，并保留进程会话用于后续轮询，确保用户有足够时间扫码。禁止使用 30 秒业务请求超时终止授权进程；禁止改用交互式 `wecom-cli init`，禁止尝试打开浏览器。
...
| 原始命令结果 | 处理 |
|---|---|
| 明确提示 `init`、未登录、未授权、请先授权或缺少配置缓存 | 启动 `init`，使用 120 秒授权等待并轮询 |
| `850001` / `invalid api key` / API Key 配置错误 | 不启动 `init`，提示管理员检查配置 |
| 权限不足 | 返回“您暂无权限执行该操作”，不启动 `init` |
| 参数、对象、接口或网络错误 | 按业务错误处理，不启动 `init` |
