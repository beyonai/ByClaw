...
2. 运行：
   ...
   ```
   禁止改用交互式 `wecom-cli init`，禁止尝试打开浏览器。
...
4. 告知用户使用企业微信打开该 Markdown 授权链接完成授权，并等待 `wecom-cli init --noninteractive --no-open` 返回成功。
   禁止跳过等待步骤，禁止用任何自动化方式代替用户授权。
...
6. 如果初始化命令提示授权超时，且用户仍要继续当前任务，则重新运行一次 `wecom-cli init --noninteractive --no-open`，返回新的 Markdown 授权链接并继续等待。

7. 授权成功后，重新执行第 1 步记录的原始 `wecom-cli` 命令。

8. 如果重试原始命令仍返回初始化/授权类错误，最多再执行一轮完整的 Step 1-7；第二轮后仍失败时，停止重试，返回最新错误并说明授权未完成。禁止伪造成功结果。
