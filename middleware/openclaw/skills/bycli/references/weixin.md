...
- `status: draft created` is the only API success. A confirmed API failure that proves no draft was created—including token acquisition, AppID/AppSecret, cover or body-image upload, `40164` IP allowlist, or an explicit nonzero `draft/add` error—triggers exactly one browser-mode attempt with the same approved title, body, content format, author, summary, and cover. Preserve the sanitized API error, remove `--appid` and `--appsecret`, perform `doctor` followed immediately by `daemon status`, and execute the fallback through `scripts/weixin-login-gate.mjs` with the same task state directory plus `--site-session persistent --keep-tab true`. Do not ask for confirmation between the two attempts.
...
- Every browser-backed command path in this reference must be executed through `scripts/weixin-login-gate.mjs`, never by invoking the underlying browser path directly. This includes the browser branch of a conditional command. The `weixin-login-gate.mjs` runner is the executable owner of the login-gate budget. For collection work, use `<collection-session-dir>/.runtime/weixin-login-gates` as `--state-dir`; initialize that collection before browser preflight. For a direct non-collection task, create one unique task directory below the current Agent Session Root and keep the same directory for the complete logical operation.
- Initial execution shape: `node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin <command> ...`. Only after the user explicitly says that human verification was completed in the retained tab may the single rerun add `--verification-confirmed true` before `--`.
...
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin articles '<fakeid>' --limit <n> --max-pages <n> --auth-source browser --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin sougousearch '<query>' --page 1 --limit <n> --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin collections --limit <n> --max-pages <n> --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin collection-detail '<collectionId>' --max-pages <n> --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin drafts --limit <n> --timeout 60 --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin published '<optional title or URL>' --limit <n> --max-pages <n> --timeout 30 --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin download --url '<article-url>' --output '<directory>' --download-images true --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin save-articles '<fakeid>' --limit <n> --max-pages <n> --output '<directory>' --auth-source browser --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin download-publish-data '<exact article URL>' --date YYYY-MM-DD --output '<directory>' --max-pages <n> --timeout 60 --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin download-publish-data '<exact article title>' --output '<directory>' --max-pages <n> --timeout 60 --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin create-draft --title '<title>' --content-file '<article.html>' --content-format html --author '<author>' --cover-image '<cover.jpg>' --summary '<summary>' --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin create-draft '<text body>' --title '<title>' --cover-image '<cover.jpg>' --dry-run true --site-session persistent --keep-tab true -f json
...
```bash
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin articles '<fakeid>' --name '<proven exact account name>' --limit <n> --max-pages <n> --auth-source browser --site-session persistent --keep-tab true -f json
node scripts/weixin-login-gate.mjs --state-dir '<absolute-state-dir>' -- bycli weixin save-articles '<fakeid>' --name '<proven exact account name>' --limit <n> --max-pages <n> --output '<directory>' --auth-source browser --site-session persistent --keep-tab true -f json
```
