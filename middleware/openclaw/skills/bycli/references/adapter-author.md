...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────────────────────┐
│ 原始请求重放                              │── 不一致 → 回字段解码 / verify
│ 用新 adapter 跑用户最初的那个请求；       │
│ 有降级驱动数据则比对，没有则比网页实际值  │
└──────────────────────────────────────────┘
  │ 一致
  ▼
┌──────────────────────────┐
...
[ ] 10. bycli browser verify：
        ...
        [ ] 再跑一次确认
[ ] 11. 字段值 vs 网页肉眼比对
[ ] 12. 原始请求重放：用新 adapter 跑用户最初的那个请求
        [ ] 比对基准：有降级驱动数据则比降级所得，没有（从零写）则比网页实际值 / 用户原始诉求
        [ ] 一致才算 adapter 可用；不一致 → 回字段解码 / verify 排查，不可直接宣告可用
        [ ] 把重放结果展示给用户（顺带正式交付一次原始诉求）
[ ] 13. 回写站点记忆（schema 见 site-memory.md）：
        [ ] endpoints.json
...
| Step 12 | 重放结果与降级驱动所得不一致 | 字段映射 / 参数错，回 Step 7 字段解码或 Step 10 verify |
