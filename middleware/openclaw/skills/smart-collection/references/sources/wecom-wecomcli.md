...
企微采集意图由 `smart-collection` 统一编排。命中本桥接后，先声明“委派采集模式”，再加载并遵循
`wecomcli` skill 及匹配的子 Skill。`smart-collection` 负责采集目录、`raw/`、`markdown/`、
...
- 来源执行器不得询问或执行 `入库 / 知识整理 / 跳过`；仅由 `smart-collection` 在采集后执行该选择。
