...
钉钉采集意图由 `smart-collection` 统一编排。命中本桥接后，先声明“委派采集模式”，再加载并遵循
`dws` skill。`smart-collection` 负责采集意图、产物目录、规范化契约和后处理；`dws` 负责钉钉产品
...
`smart-collection` 按 [post-processing.md](../post-processing.md) 统一完成。
