...
本流程与具体执行器无关。来源执行器只负责采集；采集编排器 `smart-collection` 统一负责持久化、预览、选择、状态和收尾。
...
状态与清理必须通过 `scripts/smart-collection-post-processing.mjs`，不得由 Agent 手工修改 JSON 或猜测关联文件：

```text
node scripts/smart-collection-post-processing.mjs inspect --session-dir <dir>
node scripts/smart-collection-post-processing.mjs mark-materialized --session-dir <dir> --item-json-file <file>
node scripts/smart-collection-post-processing.mjs record-run --session-dir <dir> --run-json-file <file>
node scripts/smart-collection-post-processing.mjs cleanup --session-dir <dir> --run-id <id>
node scripts/smart-collection-post-processing.mjs unlock-stale --session-dir <dir>
node scripts/smart-collection-post-processing.mjs set-retention --session-dir <dir> --keep true|false
node scripts/smart-collection-post-processing.mjs rewrite-image-links --session-dir <dir> --resource-id <数字员工资源ID>
```
...
```text
node scripts/smart-collection-post-processing.mjs rewrite-image-links --session-dir <dir> \
