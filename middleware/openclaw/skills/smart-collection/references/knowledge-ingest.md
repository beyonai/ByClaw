...
采集编排器 `smart-collection` 拥有入库编排与确认边界；`by-knowledge-manager` 是底层入库执行器。通过 `smart-collection` 内的
`scripts/smart-collection-ingest.mjs` 调用它，不得把入库所有权移回 `bycli`。
...
通过 `smart-collection` 处理当前采集批次时，选中正文只能来自 `sanitized/items/*.md`。缺失净化正文时先按
...
脚本路径均相对 `smart-collection` Skill 根目录：

`node scripts/smart-collection-ingest.mjs <command> ...`
...
具体参数以 `node scripts/smart-collection-ingest.mjs --help` 为准。命令和示例中只使用资源 ID、
目录与本地文件路径等非敏感占位值。
