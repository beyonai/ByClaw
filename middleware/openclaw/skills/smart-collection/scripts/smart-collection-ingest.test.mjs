...
const SCRIPT = path.join(TEST_DIR, "smart-collection-ingest.mjs");
...
async function testHelpUsesMigratedIdentityAndCanonicalInputFirst() {
  ...
  assert.match(result.stdout, /smart-collection-ingest/);
...
async function testCanonicalCollectionResultPreservesMarkdownFrontmatter() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-fixture-"));
...
async function testCanonicalCollectionResultRejectsInvalidContractShapes() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-contract-"));
...
async function testCanonicalCollectionResultAllowsEmptyMaterializedView() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-empty-view-"));
  const fixturePath = path.join(fixtureDir, "collection-result.json");
...
async function testCanonicalCollectionResultRejectsUnsafeOrMissingMarkdownPaths() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-path-safety-"));
...
async function testCanonicalCollectionResultRejectsMismatchedMarkdownAndFileName() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-mismatch-"));
  const fixturePath = path.join(fixtureDir, "collection-result.json");
  fs.mkdirSync(path.join(fixtureDir, "sanitized/items"), { recursive: true });
...
async function testCanonicalIngestUsesValidatedRootArtifactDespiteDirectoryOverrides() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-canonical-ingest-"));
  const overrideDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-override-"));
...
async function testCanonicalActualIngestKeepsValidatedPathPrivate() {
  ...
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-private-ingest-"));
  const overrideDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-private-override-"));
...
async function testCanonicalInputPrecedesLegacyAndLegacyRemainsSupported() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-precedence-"));
...
async function testLocalResourceEnforcesFileAndBatchLimits() {
  const server = createServer();
  const port = await server.listen();
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-limits-"));
...
async function testIngestInlineMarkdownUploadsThroughManager() {
  ...
  try {
    ...
    assert.match(result.json?.uploaded?.generatedFiles?.[0] || "", /smart-collection-ingest-/);
    const upload = server.requests.find((r) => r.url === "/byaiService/datasetController/uploadFiles");
...
async function testIngestPropagatesOverwriteConfirmationAsNonTerminalState() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-conflict-manager-"));
...
async function testIngestRejectsSuccessfulManagerExitWithoutJsonContract() {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-empty-manager-"));
  const managerPath = path.join(fixtureDir, "manager.mjs");
...
console.log("smart-collection-ingest tests passed");
