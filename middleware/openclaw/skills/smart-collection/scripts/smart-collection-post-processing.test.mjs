...
const scriptPath = resolve(dirname(new URL(import.meta.url).pathname), 'smart-collection-post-processing.mjs');
...
async function createSession(ids = ['a', 'b'], metadataValue) {
  const root = await mkdtemp(join(tmpdir(), 'smart-collection-post-processing-'));
  await Promise.all([
...
async function testStaleLockRecoverySkipsIfLockChangedDuringRecovery() {
  const root = await createSession(['a']);
  const lockPath = join(root, '.smart-collection-post-processing.lock');
...
async function testUnlockStaleRejectsMalformedLockAndRemovesDeadOwner() {
  ...
  try {
    const lockPath = join(malformedRoot, '.smart-collection-post-processing.lock');
  ...
  try {
    const lockPath = join(staleRoot, '.smart-collection-post-processing.lock');
    await writeFile(lockPath, JSON.stringify({
      pid: 99999999,
      createdAt: new Date(0).toISOString(),
      ownerId: 'dead-owner',
...
console.log('smart-collection post-processing tests passed');
