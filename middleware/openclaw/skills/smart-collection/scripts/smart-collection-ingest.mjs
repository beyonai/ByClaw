...
function sanitizeFileName(value, fallback) {
  const raw = firstNonEmpty(value, fallback, "smart-collection-output");
  const safe = raw
    .replace(/[/\\?%*:|"<>]/g, "-")
    .replace(/\s+/g, "-")
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "")
    .slice(0, 120) || "smart-collection-output";
  return safe.toLowerCase().endsWith(".md") ? safe : `${safe}.md`;
}

function sanitizeResourceFileName(value, fallback, source) {
  const raw = firstNonEmpty(value, fallback, "smart-collection-resource");
  let safe = raw
    .replace(/[/\\?%*:|"<>]/g, "-")
    .replace(/\s+/g, "-")
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "")
    .slice(0, 120) || "smart-collection-resource";
...
function normalizeResourceCandidate(raw, args, index) {
  ...
  const fileName = sanitizeResourceFileName(
    ...
    fileNameFromUrl(source) || path.basename(String(source)) || `smart-collection-resource-${index + 1}`,
    source,
  );
...
function normalizeItem(rawItem, args, index) {
  ...
  const fallbackName = firstNonEmpty(
    ...
    `smart-collection-output-${index + 1}`,
  );
  return {
    title,
...
function markdownItemsFromFiles(args) {
  ...
  return files.map((filePath, index) => {
    ...
    const fileName = sanitizeFileName(path.basename(filePath), `smart-collection-output-${index + 1}`);
    return {
...
function buildCollectionResult(args, stdinValue) {
  ...
  collectionResult.items = collectionResult.items.map((item, index) => ({
    ...
    fileName: input.type === "collection-result-file"
      ? item.fileName
      : sanitizeFileName(item.fileName, `smart-collection-output-${index + 1}`),
  }));
...
function prepareMarkdownUploadFiles(args, payloads) {
  ...
  for (const [index, item] of payloads.markdownFiles.entries()) {
    ...
    if (!tempDir) {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-collection-ingest-"));
    }
    const fileName = sanitizeFileName(item.fileName, `smart-collection-output-${index + 1}.md`);
...
async function printHelp() {
  render({
    ...
    examples: [
      "node smart-collection-ingest.mjs normalize --collection-result-file /tmp/smart-collection-result.json --knowledge-base-resource-id 90001",
      "node smart-collection-ingest.mjs ingest --dry-run --markdown-file article.md --knowledge-base-resource-id 90001 --directory-path /imports",
      "node smart-collection-ingest.mjs ingest --markdown-file article.md --knowledge-base-resource-id 90001 --directory-path /imports --confirmed-knowledge-base-resource-id 90001 --confirmed-directory-path /imports",
      "node smart-collection-ingest.mjs ingest --markdown-file article.md --knowledge-base-resource-id 90001 --directory-path /imports --confirmed-knowledge-base-resource-id 90001 --confirmed-directory-path /imports --confirmed-overwrite-path /imports/article.md",
      "node smart-collection-ingest.mjs upload-doc --file-path report.pdf --knowledge-base-resource-id 90001 --directory-path /imports --confirmed-knowledge-base-resource-id 90001 --confirmed-directory-path /imports",
    ],
