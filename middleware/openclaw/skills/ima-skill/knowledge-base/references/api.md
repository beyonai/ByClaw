> **ByClaw Runtime notice — non-executable reference:** This upstream API document is retained for field lookup only. Do not construct HTTP calls, use `curl`, or use an `ima_api` helper from this document. Use the corresponding `ima wiki … --json` command instead.
