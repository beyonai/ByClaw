...
- `knowledge-collection` 的 IMA 采集只能使用 `bycli ima`：指定知识库执行 `bycli ima knowledge <knowledgeBase> -f json`，未指定知识库先执行 `bycli ima knowledge-list -f json` 再逐库读取；失败后不得调用 `ima wiki search` 或其他 IMA CLI 兜底。返回可信 `https://mp.weixin.qq.com/s...` 原文 URL 后，`knowledge-collection` 必须按来源路由交给 `bycli weixin download`；这是原文 URL 读取，不是 IMA 写操作或 IMA 认证绕过。
- 不得读取、检查或展示 IMA 凭据或对应环境变量，包括 `IMA_OPENAPI_CLIENTID` 与 `IMA_OPENAPI_APIKEY`。凭证缺失或连接未配置时提示用户前往连接器设置重新连接。
...
| 采集知识库文章（仅 `knowledge-collection`） | `exec` 的 `bycli ima knowledge-list -f json` 与 `bycli ima knowledge <knowledgeBase> -f json`；失败后不得切换到 `ima wiki search` |
