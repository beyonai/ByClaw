...
**OpenClaw 布局：** 本 skillpack  vendored 在
`${OPENCLAW_SKILL_GBRAIN_DIR:-/app/skills/gbrain}/references/` 下。上游 gbrain
仓库仍使用 `skills/` 目录名；OpenClaw 内所有交叉引用统一写 `references/`（与
父级 `SKILL.md` 一致）。
...
Discover skills at runtime by walking every `references/<slug>/SKILL.md` here and
...
Read the entire `references/<slug>/SKILL.md` file. Follow its `## Phases`,
