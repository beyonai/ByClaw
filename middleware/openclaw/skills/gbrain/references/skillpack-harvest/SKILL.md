...
writes_pages: false
writes_to:
  - references/<harvested-slug>/   # OpenClaw 本地 bundle；上游 gbrain 仓库为 skills/<slug>/
  - openclaw.plugin.json
---

# skillpack-harvest — Editorial workflow for lifting host skills into gbrain

> **路径约定（OpenClaw）：** 本 skillpack  vendored 在 `references/` 下，文档内
> 交叉引用用 `references/<slug>/SKILL.md`。Host 仓库与上游 gbrain 仓库仍使用
> `skills/<slug>/`；`gbrain skillpack harvest` CLI 写入上游时也是 `skills/`。
...
1. `skills/<harvested-slug>/SKILL.md`（上游 gbrain 树；OpenClaw 本地为
...
5. **routing-eval.jsonl examples are scrubbed**
   - Open host `skills/<slug>/routing-eval.jsonl`. Every `intent` field
...
4. Review the diff: `cd <gbrainRoot> && git diff -- skills/<slug>/`
   （OpenClaw 本地 vendored 副本：`references/<slug>/`）
...
- gbrain 上游 `skills/<slug>/` — every file in the host skill dir
  (copy). OpenClaw 本地 bundle 同步为 `references/<slug>/`。
