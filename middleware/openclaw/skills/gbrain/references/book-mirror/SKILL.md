...
6. PDF       → Optional: render via references/brain-pdf for delivery.
```
...
no embedded text, fall back to OCR via `references/brain-pdf` or another
vision tool.
...
After the brain page is written, render to PDF using `references/brain-pdf`:

```bash
gbrain put_page  # already done by the CLI; nothing to add here
# Then invoke brain-pdf:
# (see references/brain-pdf/SKILL.md for the make-pdf invocation)
```
...
- `references/brain-pdf/SKILL.md` — render the personalized page to PDF.
- `references/strategic-reading/SKILL.md` — read a book through a specific
  problem-lens instead of personalizing to the whole reader.
- `references/article-enrichment/SKILL.md` — same shape applied to articles
  rather than books.
