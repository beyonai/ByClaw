...
□ 7.  Resolver trigger   — entry in references/RESOLVER.md with real user trigger phrases
...
gbrain eval cross-modal \
  ...
  --output references/<slug>/SKILL.md
```
...
1. Add to references/RESOLVER.md with trigger phrases users ACTUALLY type
2. Resolver eval: feed triggers, assert correct routing
3. Check-resolvable:
   - Skill reachable from references/RESOLVER.md (not orphaned)
...
gbrain skillify check references/<slug>/scripts/<slug>.mjs --json | \
...
Phase 5: Add "summarize this PR" trigger to references/RESOLVER.md
...
1. **The skill tree on disk.** `references/<slug>/SKILL.md`, `scripts/<slug>.mjs`,
