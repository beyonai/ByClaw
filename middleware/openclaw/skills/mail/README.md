...
Mail 内置技能注册、资源授权和七个邮箱连接器的初始化 DML 统一维护在 V0.5.0，V0.4.0 不再包含这些 DML。连接器种子按编码去重，保留原 OAuth2 授权与凭据投影配置，并应用 V0.5.0 的表单配置和停用状态；技能注册继续沿用原 advisory lock 键，兼容旧版本并发重放。

迁移使用普通 SQL，不使用 `DO` 匿名块。所有语句须在同一连接中顺序执行；会话锁覆盖自动提交模式下的技能注册，事务锁保证外层事务提交前不被并发重放。失败时应停止执行并关闭连接，释放会话锁；自动提交可能留下已完成的步骤，可重跑收敛，需要整体回滚时由调用方提供事务。
