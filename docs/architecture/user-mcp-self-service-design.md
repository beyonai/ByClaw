...
> 版本化工具快照和调用网关。当前临时将发现到的全部工具标记为 `READ`，尚未启用工具级读写风险治理；
> SSE 因动态 endpoint 尚无法做到 Socket 级 DNS 固定而暂不开放；
...
当前阶段暂未实现上述工具分级，所有发现到的工具统一标记为 `READ`。这是一项显式的临时兼容策略，不代表远端工具
已经过安全审计；后续上线写操作治理前必须恢复默认最小权限模型。
...
当前实现暂时把所有发现工具写入为 `READ/SYSTEM_DEFAULT`，因此都会进入图中的 `READ` 分支；工具级风险分类和
`ToolInvocationGrant` 仍是阶段二必须补齐的安全能力。
...
目标态的风险等级优先使用可信发布信息和 MCP 工具注解，其次才使用名称/描述分类，自动结果允许管理员人工覆盖。
`WRITE/UNKNOWN` 必须使用第 7.3 节定义的单次 Grant；资源所有者不能自行降低工具风险。

当前阶段不提供管理员只读规则参数，工具发现统一写入 `READ/SYSTEM_DEFAULT`。该临时策略取消了工具级写操作拦截，
但资源归属、端点 allowlist、实例启用状态、凭据绑定、定义版本和输入 Schema 校验仍必须执行。
...
- 工具清单变化时生成新快照，当前新增工具临时默认 `READ`，已删除工具立即不可调用。
...
4. 当前发现到的全部工具均以临时默认 `READ` 执行，文档明确提示工具级写操作风险治理尚未启用；
