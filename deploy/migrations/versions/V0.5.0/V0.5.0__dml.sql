...
-- ============================================================
-- 研发闭环编码环节打点：给 DEVLOOP_TASK_START_PROMPT 补 [PHASE] coder 打点要求
--
-- 背景：平台靠数字员工回复正文里的 [PHASE] 打点派生流水线环节状态
-- （见 DevloopPhaseService.MARKER）。而任务启动提示词从未要求过打点，
-- 实测线上 coder 打点数为 0，导致需求级批量集成的就绪筛选（allTasksCoded
-- 判 coder 环节是否 done）恒为空，定时集成任务永远不触发。
--
-- 用 UPDATE 追加而非 delete+insert 重建：该模板可在线编辑，各环境已有
-- 自行调整（如多 module 说明、devloop-run-governance skill），整体替换会冲掉。
-- 幂等：仅当模板里还没有 [PHASE] 时才追加。
-- ============================================================
UPDATE byai.byai_ai_prompt
SET prompt_zh_template = prompt_zh_template || E'\n- 开工前在回复正文输出 [PHASE] coder START，完成后输出 [PHASE] coder DONE；无法完成时输出 [PHASE] coder REJECT->req 原因:<具体原因>。打点必须在回复正文里，写进文件或包在代码块里无效',
    update_time = now()
WHERE prompt_code = 'DEVLOOP_TASK_START_PROMPT'
  AND prompt_zh_template IS NOT NULL
  AND prompt_zh_template NOT LIKE '%[PHASE]%';

UPDATE byai.byai_ai_prompt
SET prompt_en_template = prompt_en_template || E'\n- Output [PHASE] coder START in your reply before starting work, and [PHASE] coder DONE when finished; if you cannot finish, output [PHASE] coder REJECT->req reason:<specific reason>. Markers must appear in the reply body -- writing them to a file or wrapping them in a code block does not count',
    update_time = now()
WHERE prompt_code = 'DEVLOOP_TASK_START_PROMPT'
  AND prompt_en_template IS NOT NULL
  AND prompt_en_template NOT LIKE '%[PHASE]%';

-- 打点规范单点持有在 devloop-phase-marker skill，提示词只要求调用它，不复述格式细节。
UPDATE byai.byai_ai_prompt
SET prompt_zh_template = replace(prompt_zh_template,
        'skill：devloop-run-governance，self-developed-rules',
        'skill：devloop-run-governance，devloop-phase-marker，self-developed-rules'),
    update_time = now()
WHERE prompt_code = 'DEVLOOP_TASK_START_PROMPT'
  AND prompt_zh_template LIKE '%devloop-run-governance，self-developed-rules%';

UPDATE byai.byai_ai_prompt
SET prompt_zh_template = replace(prompt_zh_template,
        'skill：self-developed-rules',
        'skill：devloop-phase-marker，self-developed-rules'),
    update_time = now()
WHERE prompt_code = 'DEVLOOP_TASK_START_PROMPT'
  AND prompt_zh_template LIKE '%skill：self-developed-rules%'
  AND prompt_zh_template NOT LIKE '%devloop-phase-marker%';
