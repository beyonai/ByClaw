...
-- 单条匿名 PL/pgSQL 语句兼容 gsql 自动提交与 psycopg 外层事务，并使锁覆盖全部收敛操作。
DO $mail$
BEGIN
    PERFORM pg_advisory_xact_lock(hashtext('byclaw'), hashtext('V0.4.0:mail-skill'));

-- 仅修改合法 JSON 数组；NULL/非数组保持原值，非法 JSON 在显式 ::jsonb 转换时失败并回滚。
...
SET provider_code = 'mail-form', auth_mode = 'AK_SK', update_time = CURRENT_TIMESTAMP
