...
-- 初始化沙箱服务表
INSERT INTO "byai"."sandbox_service_spec" ("service_key", "spec_json", "template_json", "updated_at") VALUES ('openclaw', '{"env": {"TZ": "Asia/Shanghai", "MODEL_ID": "${MODEL_ID}", "NODE_ENV": "production", "USER_CODE": "${user_code}", "MODEL_NAME": "${MODEL_NAME}", "REDIS_HOST": "${REDIS_HOST}", "REDIS_PORT": "${REDIS_PORT}", "MODEL_ALIAS": "${MODEL_ALIAS}", "OPENCLAW_TZ": "Asia/Shanghai", "BEYOND_TOKEN": "${BEYOND_TOKEN}", "NODE_OPTIONS": "--max-old-space-size=4096", "MODEL_API_KEY": "${MODEL_API_KEY}", "DWS_CONFIG_DIR": "/by/.openclaw/.dws", "MODEL_BASE_URL": "${MODEL_BASE_URL}", "REDIS_DATABASE": "${REDIS_DATABASE}", "REDIS_PASSWORD": "${REDIS_PASSWORD}", "REDIS_USERNAME": "${REDIS_USERNAME}", "BAIYING_SESSION": "${BAIYING_SESSION}", "BAIYING_AGENT_AUTH": "${BAIYING_AGENT_AUTH}", "OPENCLAW_STATE_DIR": "/by/.openclaw", "OPENCLAW_GATEWAY_TOKEN": "${OPENCLAW_GATEWAY_TOKEN}", "BYCLAW_SANDBOX_FILE_VOLUME_ROOT": "${BYCLAW_SANDBOX_FILE_VOLUME_ROOT}","DATACLOUD_DB_TYPE": "${DB_TYPE}","DATACLOUD_DB_HOST": "${DB_HOST}","DATACLOUD_DB_PORT": "${DB_PORT}","DATACLOUD_DB_DATABASE": "${DB_DATABASE}","DATACLOUD_DB_SCHEMA": "${DB_SCHEMA}","DATACLOUD_DB_USER": "${DB_USER}","DATACLOUD_DB_PASS": "${DB_PASS}","DATACLOUD_DB_PASSWORD": "${DB_PASS}","DEMO_SCHEMA": "${DEMO_SCHEMA}","BE_DOMAINNAME": "ByaiService"}, "image": "ghcr.io/beyonai/byclaw/byclaw-openclaw:main", "ports": [{"port": 8080, "protocol": "http"}, {"port": 8081, "protocol": "http"}, {"port": 9222, "protocol": "http"}, {"port": 5901, "protocol": "http"}, {"port": 18789, "protocol": "http"}], "startup": {"entrypoint": ["node", "dist/index.js", "gateway", "--bind=lan", "--port=8080", "--allow-unconfigured", "--verbose"]}, "volumes": [{"key": "base", "scope": "PRIVATE", "subPath": "byclaw-${user_code}/by", "hostPath": "${BYCLAW_SANDBOX_FILE_VOLUME_ROOT}", "readOnly": false, "mountPath": "/by"}, {"scope": "PUBLIC", "subPath": "byclaw/resource", "hostPath": "${BYCLAW_SANDBOX_FILE_VOLUME_ROOT}", "readOnly": false, "mountPath": "/by/.openclaw/byresources"}], "bootstrap": {"copyTemplate": {"copyIfMissing": true, "targetVolumeKey": "base"}}, "resourceLimits": {"cpu": "2", "memory": "4Gi"}}', '{
    "meta": {
...
-- ========== V0.0.2 (merged at 2026-07-06 10:27:58) ==========
...
-- ========== V0.0.3 (merged at 2026-07-06 10:27:58) ==========
...
INSERT INTO byai.byai_system_config (param_id, param_type, param_code, param_name, param_en_name, param_value, param_desc) VALUES (nextval('byai.seq_any_table'), null, 'INIT_DEFAULT_PROJECT_EXPERT_TEAMS_TEMPLATE', '初始化项目专家组数字员工模板', 'INIT_DEFAULT_PROJECT_EXPERT_TEAMS_TEMPLATE', e'{
	"zh_CN": [{
			"projectName": "${userName}的百应运营项目",
			"projectCode": "operation_${userCode}",
		...
		},
		{
			...
			"projectCode": "development_${userCode}",
	...
	"en_US": [{
			"projectName": "${userName} is Baiying Operation Project",
			"projectCode": "operation_${userCode}",
			"projectType": "operation",
			"description": "Baiying Operation Project. Initialization",
			"isShare": "Y"
		},
		{
			"projectName": "${userName} is Baiying Development Project",
			"projectCode": "development_${userCode}",
			"projectType": "development",
			"description": "Baiying Development Project. Initialization",
			"isShare": "Y"
		}
	]
}
