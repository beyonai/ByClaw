...
CREATE TABLE IF NOT EXISTS byai.byai_user_mcp_tool_snapshot (snapshot_id BIGINT NOT NULL PRIMARY KEY,resource_id BIGINT NOT NULL,definition_revision BIGINT NOT NULL,snapshot_version BIGINT NOT NULL,tool_name VARCHAR(255) NOT NULL,description TEXT,input_schema TEXT NOT NULL,schema_hash VARCHAR(128) NOT NULL,risk_level VARCHAR(16) NOT NULL DEFAULT 'READ',risk_source VARCHAR(32) NOT NULL DEFAULT 'SYSTEM_DEFAULT',status_cd VARCHAR(3) NOT NULL DEFAULT '00A',create_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,CONSTRAINT uk_byai_user_mcp_tool_snapshot UNIQUE (resource_id,snapshot_version,tool_name));
