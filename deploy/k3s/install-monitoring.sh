...
create_grafana_dashboard() {
    local dashboard_file="/tmp/byclaw-grafana-dashboard.json"
    ...
    cat >"$dashboard_file" <<'JSON'
{
  ...
  "dashboard": {
    "id": null,
    "uid": "byclaw-k3s-pods",
    "title": "ByClaw K3s Cluster & Pod Resources",
    "tags": ["byclaw", "k3s", "pods"],
    "timezone": "browser",
    "schemaVersion": 39,
    "version": 1,
    "refresh": "30s",
    ...
    "panels": [
      ...
      {
        "id": 4,
        "title": "Namespace Memory Working Set",
        "type": "timeseries",
      ...
      {
        "id": 5,
        "title": "Top Pod CPU Usage",
        "type": "table",
        "gridPos": {"h": 8, "w": 12, "x": 0, "y": 16},
        "targets": [
          {
            "refId": "A",
            "expr": "topk(20, sum by (namespace, pod) (rate(container_cpu_usage_seconds_total{container!=\"\",pod!=\"\"}[5m])))",
            "format": "table",
            "instant": true
          }
        ],
      ...
      },
      {
        ...
        "targets": [
          {
            "refId": "A",
            "expr": "topk(20, sum by (namespace, pod) (container_memory_working_set_bytes{container!=\"\",pod!=\"\"}))",
            "format": "table",
            "instant": true
          }
        ],
...
JSON
    ...
    if curl -fsS --retry 20 --retry-delay 3 \
        ...
        --data-binary "@${dashboard_file}" >/dev/null; then
        echo "    dashboard: ${grafana_url}/d/byclaw-k3s-pods/byclaw-k3s-cluster-and-pod-resources"
    else
        echo "Warning: failed to import Grafana dashboard through ${grafana_url}; monitoring stack is still installed." >&2
    fi
}
...
echo "    Prometheus: ${MONITORING_EXTERNAL_BASE_URL}/prometheus"
echo "    Grafana: ${MONITORING_EXTERNAL_BASE_URL}/grafana"
echo "    Grafana dashboard: ${MONITORING_EXTERNAL_BASE_URL}/grafana/d/byclaw-k3s-pods/byclaw-k3s-cluster-and-pod-resources"
