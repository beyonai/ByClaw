...
async def s3_client(pg_dsn, redis_url) -> AsyncIterator:  # pylint: disable=unused-argument,redefined-outer-name
    """App wired with real backends + 3 KB configs seeded in Redis."""
    ...
    for kn_code, cfg in _KBS.items():
        ...
        await redis_client.set(key, json.dumps(cfg))
