...
字段和状态值在本文的状态机、停止流程及 API 章节中同步说明；实现以现有前端已使用的
`byclaw.task-plan/v1` 合同为准。
...
OpenClaw 插件运行时注入可信的 `sessionId`、`messageId`、`sourceRuntime`、`sourceRunId` 和
...
只要计划仍为 `ACTIVE`，Runtime 就保持 SDK 完成门关闭，并在同一 Leader Session 内继续执行。
连续三轮计划版本没有推进时，Run 转为 FAILED，Runtime 自动把当前任务和后续任务收口。

数字员工委派与任务计划相互独立：

- `delegate_agent` 不接收 `planId`、`taskId`、`taskPosition` 或任务状态；
