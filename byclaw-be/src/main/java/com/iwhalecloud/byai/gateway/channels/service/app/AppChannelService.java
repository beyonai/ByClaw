package com.iwhalecloud.byai.gateway.channels.service.app;
...
public class AppChannelService implements ChannelService {
    ...
    public void chat(AssistantChatDto assistantChatDto, OutputStream outputStream) {
        log.info("App渠道处理对话请求, assistantId: {}", assistantChatDto.getAssistantId());
        try {
            assistantChatService.chat(assistantChatDto, outputStream, null);
        } catch (IOException e) {
            log.error("App渠道对话处理异常, assistantId: {}", assistantChatDto.getAssistantId(), e);
    ...
    public boolean validateRequest(AssistantChatDto assistantChatDto) {
        // App 渠道特定的参数验证逻辑
        return assistantChatDto != null && assistantChatDto.getAssistantId() != null;
    }
}
