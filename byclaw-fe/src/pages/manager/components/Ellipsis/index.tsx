...
const EllipsisText = ({ text, length, tooltip, tooltipProps, fullWidthRecognition, ...other }) => {
  ...
  if (tooltip) {
    return (
      <Tooltip
        ...
        title={text}
        color="#fff"
        styles={{ body: { color: 'rgba(0,0,0,0.65)' } }}
        {...tooltipProps}
      >
        <span>
          {displayText}
...
export default class Ellipsis extends Component<EllipsisProps, EllipsisState> {
  ...
  render() {
    ...
    const { children, lines, length, className, tooltip, tooltipProps, fullWidthRecognition, ...restProps } =
      this.props;
    ...
    if (isSupportLineClamp) {
      const style = `#${id}{-webkit-line-clamp:${lines};-webkit-box-orient: vertical;white-space:normal}`;
