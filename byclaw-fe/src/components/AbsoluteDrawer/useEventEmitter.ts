...
function useActionEffect() {
  ...
  useEffect(() => {
    ...
    EventEmitter.on('beyond-absolute-driver-message', onMessage);
    return () => {
      EventEmitter.off('beyond-absolute-driver-message', onMessage);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    return () => {
      EventEmitter.off('beyond-absolute-driver-open-type', drawerTypeHandler);
    };
  }, []);
