...
const Assistant = React.memo((props: { showAssistantTips?: boolean }) => {
  ...
  // 标记是否有一次播放请求正在等待视频可播;避免 canplay 反复触发时重复执行 play。
  const pendingPlayRef = useRef(false);
  // 区分等待中的播放请求是否来自 userInfo 就绪后的自动播放,用于自动播放只计一次。
  ...
  const playAssistantVideo = useCallback(() => {
    ...
    const isAutoPlay = pendingAutoPlayRef.current;
  ...
  const requestAssistantVideoPlay = useCallback(
    (isAutoPlay = false) => {
      ...
      pendingAutoPlayRef.current = isAutoPlay;
  ...
  }, [tipsList, requestAssistantVideoPlay]);
  ...
  return (
    <div className={styles.assistant}>
      <div className={styles.wrapper}>
        <img
          alt=""
          key={getAssistantIcon}
          src={getAssistantIcon}
          style={{ display: videoEnded ? 'block' : 'none', width: '100%' }}
        />
        <video
          preload="auto"
          key={getAssistantVideo}
          ref={videoRef}
